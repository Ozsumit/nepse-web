...
Learn More: https://nextjs.org/docs/messages/node-module-in-edge-runtime`;
  ...
  `);
      ...
      let bt = ["/login", "/signup", "/api/health"];
      function bu(a10) {
        let { pathname: b2 } = a10.nextUrl;
        if (bt.some((a11) => b2.startsWith(a11)) || b2.startsWith("/_next/") || b2.startsWith("/static/") || b2.includes(".")) return S.next();
        if (!a10.cookies.get("auth_token")?.value && (b2.startsWith("/dashboard") || b2.startsWith("/admin"))) {
          let c2 = new URL("/login", a10.url);
          return c2.searchParams.set("redirect", b2), S.redirect(c2);
        }
        return S.next();
      }
      let bv = { matcher: ["/((?!api|_next/static|_next/image|favicon.ico).*)"] };
